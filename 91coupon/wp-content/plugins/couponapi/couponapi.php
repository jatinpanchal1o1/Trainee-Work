...
function couponapi_get_troubleshootings() {
	...
	if(couponapi_is_location_supported(get_template()) and $configs['import_locations']=='On') {
		if($configs['theme']=='clipmydeals' and !$configs['location_taxonomy']) {
			$troubleshooting['Locations'] = array(
												"status"=>"no",
		...
		if($configs['theme']=='clipmydeals' and !$configs['location_taxonomy']) {
			$troubleshooting['Locations'] = array(
												"status"=>"warning",
												"message"=>__("Your Theme supports Location Taxonomy, but it is disabled on your theme",'couponapi'),
			);
		} else {	
			$troubleshooting['Locations'] = array(
												"status"=>"warning",
...
{
	$supported = array('clipmydeals', 'couponis', 'couponer', 'mts_coupons', 'clipper', 'couponxl', 'couponxxl', 'rehub', 'rehub-theme', 'wpcoupon', 'wp-coupon', 'CP', 'cp', 'CPq', 'couponhut');
...
{
	return $theme_name == 'clipmydeals' or (in_array($theme_name, array('couponxl', 'couponxxl', 'rehub', 'rehub-theme', 'couponhut')) and $use_logos == 'on');
}
...
function couponapi_admin_menu() {
	...
	add_submenu_page("couponapi", "Settings",	"Settings", 	'manage_options', "couponapi", "couponapi_display_settings");
	add_submenu_page("couponapi", "Coupon API Brandlogos Settings","Brandlogos", 'manage_options', "couponapi-brandlogos-settings","couponapi_display_brandlogos_settings");
	add_submenu_page("couponapi", "Coupon API CSV Upload",	"CSV Upload", 	'manage_options', "couponapi-file-upload", "couponapi_display_file_upload");
	add_submenu_page("couponapi", "Coupon API Logs", 		"Logs", 		'manage_options', "couponapi-logs", 		"couponapi_display_logs");
	add_submenu_page("couponapi", "Coupon API Troubleshoot","Troubleshoot", 'manage_options', "couponapi-troubleshoot","couponapi_display_troubleshoot");
}
...
function couponapi_scripts() {
	global $wpdb;
	$config = $wpdb->get_row("SELECT value as is_premium FROM {$wpdb->prefix}couponapi_config WHERE name = 'is_premium'");
	if($wpdb->num_rows != 0 and $config->is_premium) {
		wp_enqueue_script('couponapi-scripts', plugins_url('js/scripts.min.js', __FILE__ ));
	}
}
add_action( 'wp_enqueue_scripts', 'couponapi_scripts' );
