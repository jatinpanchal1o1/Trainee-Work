...
function couponapi_delete_offers() {
	...
	$count_suspended = 0;
	$post_type = $wpdb->get_var("SELECT `post_type` FROM `{$wp_prefix}posts` WHERE `ID` IN (SELECT `post_id` FROM  `{$wp_prefix}postmeta` WHERE `meta_key` = 'capi_id')") ?: '[post_type]';
	
	wp_defer_term_counting( true );
	$wpdb->query( 'SET autocommit = 0;' );
	
    $wpdb->query("DELETE FROM `{$wp_prefix}posts` WHERE `ID` IN (SELECT `post_id` FROM  `{$wp_prefix}postmeta` WHERE `meta_key` = 'capi_id')");
    ...
    $wpdb->query("UPDATE `{$wp_prefix}term_taxonomy` `tt` SET `count` = ( SELECT count(p.ID)  FROM  `{$wp_prefix}term_relationships` `tr` LEFT JOIN `{$wp_prefix}posts` `p` ON (`p`.`ID` = `tr`.`object_id` AND `p`.`post_type` = '$post_type' AND `p`.`post_status` IN ('publish', 'future')) WHERE `tr`.`term_taxonomy_id` = `tt`.`term_taxonomy_id` )");
