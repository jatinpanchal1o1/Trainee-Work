...
function couponapi_save_json_to_db($feedURL) {
	...
	if(!$response['result']){
		...
		$coupon_keys = array('offer_id', 'title', 'description', 'label', 'code', 'featured', 'source', 'url', 'deeplink', 'affiliate_link', 'cashback_link', 'image_url','brand_logo', 'type', 'store', 'merchant_home_page', 'categories', 'start_date', 'end_date', 'status');
...
function couponapi_save_csv_to_db($feedFile) {
	...
	if (($handle = fopen($feedFile, 'r')) === FALSE){
		...
		$topheader_db = array('offer_id', 'title', 'description', 'label', 'code', 'featured', 'source', 'url', 'deeplink', 'affiliate_link',
...
function couponapi_process_batch() {
	...
	$result = $wpdb->get_results("SELECT * FROM ".$wp_prefix."couponapi_config WHERE name IN ('import_images','cashback','batch_size','brandlogos_key', 'use_grey_image', 'use_logos', 'size', 'import_locations')");
	...
	foreach($coupons as $coupon) {
		...
		if($coupon->status == 'new' or $coupon->status == '') {
			...
			foreach($str_names as $str) {
				...
				// @4.4 TODO: Remove in later versions
				$storeTerm = get_term_by('name', $str, 'stores');
				$storeTermmeta = array_merge(get_option("taxonomy_term_{$storeTerm->term_id}", array()), array('store_url' => $coupon->merchant_home_page));
				update_option("taxonomy_term_{$storeTerm->term_id}", $storeTermmeta);
			...
			if($config['import_images']=='On' and $config['use_logos'] != 'on' and !empty($coupon->image_url)) {
				update_post_meta($post_id, 'cmd_image_url', $coupon->image_url);
			}
	...
	foreach($coupons as $coupon) {
		...
		if($coupon->status == 'new' or $coupon->status == '') {
			...
			foreach($store_names as $str) {
				...
				// @4.4 TODO: Remove in later versions
				$storeTerm = get_term_by('name', $str, 'stores');
				$isURLSaved = $wpdb->get_var("SELECT COUNT(*) FROM `{$wp_prefix}clpr_storesmeta` WHERE `meta_key` = 'clpr_store_url' AND `stores_id` = {$storeTerm->term_id}");
				$update_query = "UPDATE `{$wp_prefix}clpr_storesmeta` SET `meta_value` = '{$coupon->merchant_home_page}' WHERE `meta_key` = 'clpr_store_url' AND `stores_id` = {$storeTerm->term_id}";
				$insert_query = "INSERT INTO `{$wp_prefix}clpr_storesmeta` (`stores_id`, `meta_key`, `meta_value`) VALUES ({$storeTerm->term_id}, 'clpr_store_url', '{$coupon->merchant_home_page}')";
				$wpdb->query($isURLSaved ? $update_query : $insert_query);
	...
	foreach($coupons as $coupon) {
		...
		if($coupon->status == 'new' or $coupon->status == '') {
			...
			foreach($store_names as $str) {
				...
				// @4.4 TODO: Remove in later versions
				$storeTerm = get_term_by('name', $str, 'store');
				$core_values["category_website_{$storeTerm->term_id}"] = $coupon->merchant_home_page;
	...
	foreach($coupons as $coupon) {
		...
		if($coupon->status == 'new' or $coupon->status == '') {
			...
			foreach($str_names as $str) {
				...
				// @4.4 TODO: Remove in later versions
				$storeTerm = get_term_by('name', $str, 'dealstore');
				update_term_meta($storeTerm->term_id,'brand_url', $coupon->merchant_home_page);
	...
	foreach ($coupons as $coupon) {
		...
		if ($coupon->status == 'new' or $coupon->status == '') {
			...
			foreach ($store_names as $str) {
				...
				// @4.4 TODO: Remove in later versions
				$storeTerm = get_term_by('name', $str, 'coupon-store');
				update_term_meta($storeTerm->term_id,'store_url', $coupon->merchant_home_page);
	...
	foreach ($coupons as $coupon) {
		...
		if ($coupon->status == 'new' or $coupon->status == '') {
			...
			foreach (explode(',', $coupon->store) as $str) {
				...
				// @4.4 TODO: Remove in later versions
				$storeTerm = get_term_by('name', $str, 'deal_company');
				update_term_meta($storeTerm->term_id,'company_website', $coupon->merchant_home_page);
				update_term_meta($storeTerm->term_id, '_company_website', 'field_55225d80cd66e');
