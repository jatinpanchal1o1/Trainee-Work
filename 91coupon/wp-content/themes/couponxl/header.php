...
<head>
	...
	if ( is_single() || is_page() ) {
		...
		?>
		<?php if ( is_singular( array( 'deal', 'coupon', 'post' ) ) ) : ?>
...
if ( $show_notification_bar == 'yes' && ( $notification_bar_closeable == 'no' || ( $notification_bar_closeable == 'yes' && ! isset( $_COOKIE['couponxl_notification'] ) ) ) && ! empty( $notification_txt ) ):
	?>
...
<input type="hidden" class="search_page_url"
       value="<?php echo esc_url( couponxl_get_permalink_by_tpl( 'page-tpl_search_page' ) ); ?>"/>
            ...
            <div class="row">
                <div class="col-sm-6 collapse search-collapse">
                    <div class="row">
                        ...
                        <form method="get"
                              action="<?php echo esc_url( couponxl_get_permalink_by_tpl( 'page-tpl_search_page' ) ) ?>"
                              class="clearfix keyword-search <?php echo ! empty( $keyword ) ? 'keyword-search-visible' : ''; ?>">
                            <div class="input-group">
                                <input type="text" class="form-control keyword_search"
                                       value="<?php echo esc_attr( $keyword ); ?>"
                                       placeholder="<?php echo couponxl_get_option( 'keyword_search_placeholder' ); ?>"
                                       name="<?php echo esc_attr( $couponxl_slugs['keyword'] ) ?>">
                            </div>
                        </form>
						<?php
						global $couponxl_slugs;
						$location = get_query_var( $couponxl_slugs['location'] ) ?: '';
						$store    = get_query_var( $couponxl_slugs['offer_store'] ) ?: '';
						?>
                        <form method="get"
                              action="<?php echo esc_url( couponxl_get_permalink_by_tpl( 'page-tpl_search_page' ) ) ?>"
                              class="clearfix main-search <?php echo ! empty( $keyword ) ? 'main-search-hide' : ''; ?>">
                            <div class="col-sm-6">
                                <i class="fa fa-map-marker"></i>
                                <div class="input-group">
                                    <input type="text" class="form-control location-search"
                                           value="<?php echo ! empty( $location ) && is_numeric( $location ) ? get_term( $location )->name : ( ! empty( $_GET[ $couponxl_slugs['location'] ] ) ?: '' ); ?>"
                                           placeholder="<?php echo couponxl_get_option( 'top_bar_location_placeholder' ); ?>">
                                    <input type="hidden" name="<?php echo esc_attr( $couponxl_slugs['location'] ) ?>"
                            ...
                            <div class="col-sm-6">
                                ...
                                <div class="input-group">
                                    <input type="text" class="form-control store-search"
                                           value="<?php echo ! empty( $store ) && is_numeric( $store ) ? get_post( $store )->post_title : ( ! empty( $_GET[ $couponxl_slugs['offer_store'] ] ) ?: '' ); ?>"
                                           placeholder="<?php echo couponxl_get_option( 'top_bar_store_placeholder' ); ?>">
                                    <input type="hidden" name="<?php echo esc_attr( $couponxl_slugs['offer_store'] ) ?>"
                                           value="<?php echo esc_attr( $offer_store ) ?>">
                                    <div class="search-options"></div>
                                </div>
                            </div>
                        </form>
...
?>
<section class="navigation" data-enable_sticky="<?php echo 'yes' == $enable_sticky ? 'yes' : 'no' ?>">
    <div class="container">
        <div class="row">
				...
				<?php if ( ! empty( $site_logo['url'] ) ): ?>
                    <a href="<?php echo esc_url( home_url( '/' ) ); ?>" class="site-logo">
                        <img src="<?php echo esc_url( $site_logo['url'] ); ?>"
                             ...
                             height="<?php echo esc_attr( $site_logo['height'] ) ?>" alt="<?php bloginfo(); ?>">
                    </a>
				<?php else: ?>
                    <a href="<?php echo esc_url( home_url( '/' ) ); ?>" class="site-logo pt-10">
