...
if ( ! function_exists( 'pbs_is_demo' ) ) {
	function pbs_is_demo() {
		if ( 'couponxl' == get_option( 'template' ) ) {
			if ( defined( 'PBS_DEMO' ) ) {
				return true;
			}
		}
...
function couponxl_requred_plugins() {
	$plugins = array(
		...
		array(
			'name'               => esc_html__( 'CouponXL Shortcodes', 'couponxl' ),
			'slug'               => 'couponxl-shortcodes',
			'source'             => get_template_directory() . '/lib/plugins/couponxl-shortcodes.zip',
			'required'           => true,
			'version'            => '1.1',
			'force_activation'   => false,
			'force_deactivation' => false,
            ...
            <td>
                <select name="user_active_status">
                    <option <?php echo ! empty( $user_active_status ) && $user_active_status != 'active' ? 'selected="selected"' : '' ?>
                            value="inactive"><?php _e( 'Inactive', 'couponxl' ) ?></option>
                    <option <?php echo empty( $user_active_status ) || $user_active_status == 'active' ? 'selected="selected"' : '' ?>
                            value="active"><?php _e( 'Active', 'couponxl' ) ?></option>
                </select>
            </td>
        </tr>
    </table>
        ...
        <tr>
            ...
            <td>
                <select name="user_agent">
                    <option <?php echo 'yes' != $user_agent ? 'selected="selected"' : '' ?>
                            value="yes"><?php _e( 'Yes', 'couponxl' ) ?></option>
                    <option <?php echo empty( $user_agent ) || $user_agent == 'no' ? 'selected="selected"' : '' ?>
                            value="no"><?php _e( 'No', 'couponxl' ) ?></option>
                </select>
            </td>
        </tr>
    </table>
...
function couponxl_scripts_styles() {
	...
	$protocol   = is_ssl() ? 'https' : 'http';
	$google_key = couponxl_get_option( 'google-api' );
	wp_enqueue_script( 'couponxl-googlemap', $protocol . '://maps.googleapis.com/maps/api/js' . ( ! empty( $google_key ) ? esc_attr( '?key=' . $google_key ) : '' ), false, false, true );
	...
	wp_localize_script( 'couponxl-custom', 'couponxl_data', array(
		...
		'geolocation'          => couponxl_get_option( 'geolocation' ),
...
function couponxl_admin_resources() {
	...
	wp_enqueue_style( 'couponxl-jquery-ui', 'https://code.jquery.com/ui/1.11.2/themes/smoothness/jquery-ui.css' );
	wp_enqueue_script( 'jquery-ui-sortable' );
...
function couponxl_coupon_button( $post_id = '', $is_shortcode = false ) {
	...
	if ( $is_shortcode || is_front_page() ) {
		...
		$base_permalink = is_ssl() ? 'https://' : 'http://' . "$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]";
	}
...
if ( ! function_exists( 'couponxl_custom_meta_boxes' ) ) {
	function couponxl_custom_meta_boxes() {
		...
		$offer_meta   = array(
			...
			array(
				'id'      => 'offer_in_slider',
				'name'    => __( 'Offer In Slider', 'couponxl' ),
				'type'    => 'select',
				'options' => array(
					'no'  => __( 'No', 'couponxl' ),
					'yes' => __( 'Yes', 'couponxl' )
		...
		$store_meta   = array(
			...
			array(
				'id'   => 'store_instagram',
				'name' => __( 'Store Instagram Page Link', 'couponxl' ),
				'type' => 'text',
				'desc' => __( 'Input link to the store instagram page.', 'couponxl' )
			),
			array(
...
function xl_remaining_time( $expire_timestamp, $left_red = 'right' ) {
	...
	if ( $left_red == 'single' ) {
		return $remaining;
    } elseif ( $left_red == 'right' ) {
		return __( 'Expires: ', 'couponxl' ) . '<span class="red-meta">' . $remaining . '</span>';
	} else {
		return '<span class="red-meta">' . __( 'Expires: ', 'couponxl' ) . '</span>' . $remaining;
	}
}
...
function couponxl_comments( $comment, $args, $depth ) {
	...
	?>
    <!-- comment-1 -->
    <div class="media <?php echo 1 < $depth ? esc_attr( 'left-padding' ) : ''; ?>">
        <div class="comment-inner clearfix">
			<?php
			$avatar = couponxl_get_avatar_url( get_avatar( $comment, 75 ) );
			if ( ! empty( $avatar ) ): ?>
                <a class="pull-left" href="javascript:;">
                    <img src="<?php echo esc_url( $avatar ); ?>" class="media-object comment-avatar">
                </a>
			<?php endif; ?>
            <div class="media-body comment-body">
                <div class="pull-left">
                    <h4><?php comment_author(); ?></h4>
                    <span><?php echo human_time_diff( get_comment_date( 'U' ), current_time( 'timestamp' ) ) . __( ' ago', 'couponxl' ); ?></span>
                </div>
...
function couponxl_get_organized( $taxonomy ) {
	...
	if ( $sort == 'asc' ) {
		switch ( $sortby ) {
			case 'name' :
				usort( $taxonomy_organized, "couponxl_organized_sort_name_asc" );
				break;
			case 'slug' :
				usort( $taxonomy_organized, "couponxl_organized_sort_slug_asc" );
				break;
			case 'count' :
				usort( $taxonomy_organized, "couponxl_organized_sort_count_asc" );
				break;
			default :
				usort( $taxonomy_organized, "couponxl_organized_sort_name_asc" );
				break;
		...
		switch ( $sortby ) {
			case 'name' :
				usort( $taxonomy_organized, "couponxl_organized_sort_name_desc" );
				break;
			case 'slug' :
				usort( $taxonomy_organized, "couponxl_organized_sort_slug_desc" );
				break;
			case 'count' :
				usort( $taxonomy_organized, "couponxl_organized_sort_count_desc" );
				break;
			default :
				usort( $taxonomy_organized, "couponxl_organized_sort_name_desc" );
				break;
...
function couponxl_search_options() {
	...
	if ( $search_by == $couponxl_slugs['location'] ) {
		...
		$stores = $wpdb->get_results( $wpdb->prepare( "SELECT ID, post_title FROM {$wpdb->posts} AS posts WHERE posts.post_type = 'store' AND posts.post_status = 'publish' AND posts.post_title LIKE %s", "%$value%" ) );
		if ( ! empty( $stores ) ) {
...
function couponxl_get_stores( WP_REST_Request $data ) {
	global $wpdb;
	$name   = $data['name'];
	$stores = $wpdb->get_results( $wpdb->prepare( "SELECT ID, post_title FROM {$wpdb->posts} AS posts WHERE posts.post_type = 'store' AND posts.post_status = 'publish' AND posts.post_title LIKE %s", "$name%" ) );
	foreach ( $stores as $store ) {
		$store->post_title = str_replace( couponxl_ucfirst( $name ), '<strong>' . couponxl_ucfirst( $name ) . '</strong>', $store->post_title );
	}
...
add_action( 'rest_api_init', function () {
	register_rest_route( 'xl/v2', '/store/(?P<name>[w\W\s\S\d\D]+)', array(
		'methods'  => 'GET',
		'callback' => 'couponxl_get_stores',
	) );
} );
add_action( 'rest_api_init', function () {
	register_rest_route( 'xl/v2', '/store/', array(
		'callback' => function () {
			return '';
		}
	) );
} );
function couponxl_get_locations( WP_REST_Request $data ) {
	global $wpdb;
	$name      = $data['name'];
	$locations = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$wpdb->terms} AS terms LEFT JOIN {$wpdb->term_taxonomy} AS tax ON terms.term_id = tax.term_id WHERE tax.taxonomy = 'location' AND terms.slug LIKE %s", "$name%" ) );
	foreach ( $locations as $location ) {
		$location->ID         = $location->term_id;
		$location->post_title = str_replace( couponxl_ucfirst( $name ), '<strong>' . couponxl_ucfirst( $name ) . '</strong>', $location->name );
	}
...
add_action( 'rest_api_init', function () {
	register_rest_route( 'xl/v2', '/location/', array(
		'callback' => function () {
			return '';
		}
	) );
...
add_action( 'rest_api_init', function () {
	register_rest_route( 'xl/v2', '/location/(?P<name>[w\W\s\S\d\D]+)', array(
		'methods'  => 'GET',
		'callback' => 'couponxl_get_locations',
	) );
} );
...
function couponxl_show_code() {
	...
	if ( ! empty( $offer ) ) {
        ...
        <h4><?php echo esc_html( $offer->post_title ); ?></h4>
		<?php
		if ( $coupon_type == 'code' ) {
		...
		if ( $coupon_modal_content == 'content' ) {
			...
			echo wp_kses_post( $offer->post_excerpt );
		}
		?>
...
function couponxl_generate_paypal_link( $amount, $title, $desc, $uniq, $cancelUrl, $returnUrl ) {
	...
	$pdata = array(
		...
		'L_PAYMENTREQUEST_0_DESC0'       => __( 'Buying Deal', 'couponxl' ),
...
function couponxl_offer_submit_paypal_link( $offer_id, $action = 'add' ) {
	...
	$response = couponxl_buy_pay_link( $offer_id, $link_text, __( 'Processing...', 'couponxl' ), $permalink, add_query_arg( array(
		'subpage'   => $subpage,
		'action'    => $action,
		'paypal'    => 'yes',
		'subaction' => 'remove_offer',
		'offer_id'  => $offer_id
	), $permalink ), add_query_arg( array(
		'subpage'   => $subpage,
		'action'    => $action,
		'paypal'    => 'yes',
		'subaction' => 'paid_offer',
		'offer_id'  => $offer_id
	), $permalink ), $amount, false );
	...
	if ( $check_offer_availability || $is_submit ) {
		...
		if ( empty( $first_name ) || empty( $phone_number ) ) {
			...
			$values = array(
				...
				'amount'      => $amount,
				'productinfo' => $is_submit ? __( 'Submitting Offer', 'couponxl' ) : get_the_title( $offer_id ),
...
function couponxl_buy_pay_link( $offer_id, $buy_now, $generating, $permalink, $success_redirect, $cancel_redirect, $amount, $btn = true ) {
	...
	if ( ! empty( $paypal_username ) ) {
		$response                     = couponxl_generate_paypal_link( $amount, $offer->post_title, $offer->post_excerpt, uniqid( '', true ), $success_redirect, $cancel_redirect );
	...
	if ( count( $payments_available ) == '1' ) {
		...
		$modal = '';
		if ( ! empty( $payments_available['paypal'] ) ) {
			$modal .= '<div class="payment-method"><a href="' . $payments_available['paypal'] . '"><img src="' . get_template_directory_uri() . '/images/paypal.png" /></a></div>';
		}
		if ( ! empty( $payments_available['stripe'] ) ) {
			...
			$modal         .= '<div class="payment-method"><a href="javascript:;" class="stripe-payment" data-genearting_string="' . esc_attr( $generating ) . '" data-pk="' . esc_attr( $pk_client_id ) . '" data-offer_id="' . esc_attr( $offer_id ) . '" data-image="' . esc_url( $site_logo['url'] ) . '" data-name="' . esc_attr( $offer->post_title ) . '" data-description="' . esc_attr( $offer->post_excerpt ) . '" data-amount="' . $stripe_amount . '" data-currency="' . esc_attr( $currency ) . '">
				<img src="' . get_template_directory_uri() . '/images/stripe.png" />
			</a></div>';
		}
		if ( ! empty( $payments_available['skrill'] ) ) {
			...
			$modal .= '<div class="payment-method"><a href="javascript:;" class="skrill-payment">
				<img src="' . get_template_directory_uri() . '/images/skrill.png" />
		...
		if ( ! empty( $payments_available['ideal'] ) ) {
			...
			$modal .= '<div class="payment-method"><a href="javascript:;" class="submit-ideal-payment" data-genearting_string="' . esc_attr( $generating ) . '">
				<img src="' . get_template_directory_uri() . '/images/ideal.png" />
		...
		if ( ! empty( $payments_available['payu'] ) ) {
			...
			$modal .= '<div class="payment-method"><a href="javascript:;" class="payu-initiate">
				<img src="' . get_template_directory_uri() . '/images/payu.png" />
...
function couponxl_generate_paypal_offer_link( $offer_id = '' ) {
	...
	if ( ! empty( $deal_link ) ) {
		...
		if ( $check_offer_availability && empty( $deal_link ) ) {
			$amount = couponxl_get_deal_amount( $offer_id );
			echo couponxl_buy_pay_link( $offer_id, __( 'BUY NOW', 'couponxl' ), __( 'Generating voucher, please wait', 'couponxl' ), $permalink, esc_url( add_query_arg( array( 'cancel' => 'true' ), $permalink ) ), $permalink, $amount );
		} else {
...
if ( ! function_exists( 'couponxl_format_price_number' ) ) {
	function couponxl_format_price_number( $price ) {
		...
		$separator     = couponxl_get_option( 'unit_separator' );
		$separator     = isset( $separator ) && ! empty( $separator ) ? $separator : '.';
...
function couponxl_generate_voucher( $length = 10 ) {
	...
	$exists = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$wpdb->postmeta} WHERE meta_key = 'voucher_code' AND meta_value = %s", $random_string ) );
...
function couponxl_get_voucher_code( $offer, $transaction_id = '', $voucher_buyer_id = '' ) {
	...
	if ( empty( $voucher_exists ) ) {
		...
		if ( function_exists( 'couponxl_voucher_code_message' ) ) {
			// send email.
			couponxl_voucher_code_message( $offer, $voucher_buyer_id, $voucher_code );
		}
...
function couponxl_check_shopping() {
	if ( ! empty( $_GET['token'] ) && ! empty( $_GET['PayerID'] ) ) {
		...
		$permalink = couponxl_get_permalink_by_tpl( 'page-tpl_my_profile' );
		echo '<div class="alert alert-info">' . __( 'Thank you for your purchase.<br />Once Skrill confirms payment you will receive your voucher on the list of your ', 'couponxl' ) . '
				  <a href="' . esc_url( couponxl_append_query_string( $permalink, array( 'subpage' => 'my_purchases' ), array( 'all' ) ) ) . '">' . __( 'purchases', 'couponxl' ) . '.
				  </a>
			  </div>';
	} else if ( ! empty( $_GET['ideal_return'] ) ) {
		$permalink = couponxl_get_permalink_by_tpl( 'page-tpl_my_profile' );
		echo '<div class="alert alert-info">' . __( 'Thank you for your purchase.<br />Once iDEAL confirms payment you will receive your voucher on the list of your ', 'couponxl' ) . '
				  <a href="' . esc_url( couponxl_append_query_string( $permalink, array( 'subpage' => 'my_purchases' ), array( 'all' ) ) ) . '">' . __( 'purchases', 'couponxl' ) . '.
				  </a>
			  </div>';
	...
	if ( ! empty( $offer ) ) {
		...
		if ( $check_offer_availability ) {
			...
			if ( $amount ) {
				...
				$payment = $iDEAL->createPayment( $bank_id, $amount * 100, __( 'Buying Deal', 'couponxl' ), $return_url, $status_url );
...
function couponxl_submit_with_ideal() {
	...
	if ( ! empty( $offer ) ) {
		...
		if ( $amount ) {
			...
			$payment = $iDEAL->createPayment( $bank_id, $amount * 100, __( 'Submitting Offer', 'couponxl' ), $return_url, $status_url );
...
function couponxl_custom_store_columns( $columns ) {
	$columns = array_slice( $columns, 0, count( $columns ) - 1, true ) + array(
			"store_logo" => __( 'Store Logo', 'couponxl' ),
...
function couponxl_custom_offer_columns( $columns ) {
	$columns = array_slice( $columns, 0, count( $columns ) - 1, true ) + array(
			"offer_store"        => __( 'Store', 'couponxl' ),
			"offer_type"         => __( 'Type', 'couponxl' ),
			"offer_expire"       => __( 'Expire Date', 'couponxl' ),
			"offer_average_rate" => __( 'Ratings', 'couponxl' ),
			"offer_category"     => __( 'Category', 'couponxl' ),
			"offer_in_slider"    => __( 'In Slider', 'couponxl' ),
			"offer_clicks"       => __( 'Clicks', 'couponxl' )
		) + array_slice( $columns, count( $columns ) - 1, count( $columns ) - 1, true );
...
function couponxl_custom_voucher_columns( $columns ) {
	$columns = array_slice( $columns, 0, count( $columns ) - 1, true ) + array(
			"voucher_deal"           => __( 'Deal', 'couponxl' ),
			"voucher_code"           => __( 'Voucher Code', 'couponxl' ),
			"voucher_owner_share"    => __( 'Owner Share', 'couponxl' ),
			"voucher_seller_share"   => __( 'Seller Share', 'couponxl' ),
			"voucher_buyer_id"       => __( 'Buyer', 'couponxl' ),
			"voucher_payment_status" => __( 'Pay To Seller', 'couponxl' ),
		) + array_slice( $columns, count( $columns ) - 1, count( $columns ) - 1, true );
...
function couponxl_save_meta_box_data( $post_id, $post ) {
	...
	if ( ! empty( $offer_discussion ) ) {
		...
		if ( function_exists( 'couponxl_send_admin_message' ) ) {
...
function couponxl_register() {
	...
	if ( pbs_is_demo() ) {
		$message = '<div class="alert alert-danger">' . __( 'Registering is not possible on demo, please login with predefined accounts', 'couponxl' ) . '</div>';
								...
								if ( ! username_exists( $username ) ) {
									if ( ! email_exists( $email ) ) {
										...
										if ( ! is_wp_error( $user_id ) ) {
											...
											$info = couponxl_send_email( $email, $registration_subject, $confirmation_message, $headers );
											...
											if ( $info ) {
												...
												$message = '<div class="alert alert-danger">' . __( 'You are registered but there was an error trying to send an email. Please contact site administrator about it.', 'couponxl' ) . '</div>';
												$success = true;
											}
										} else {
