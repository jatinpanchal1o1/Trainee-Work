	...
	class Seravo_Custom_Bulk_Action {
					...
					<script type="text/javascript">
						jQuery(document).ready(function() {
							...
							foreach ($this->actions as $action_name => $action) { ?>
								jQuery('<option>').val('<?php echo esc_attr( $action_name ); ?>').text('<?php echo esc_attr( $action["menu_text"] ); ?>').appendTo("select[name='action']");
								jQuery('<option>').val('<?php echo esc_attr( $action_name ); ?>').text('<?php echo esc_attr( $action["menu_text"] ); ?>').appendTo("select[name='action2']");

								jQuery('.search-box').append( '<a href="javascript:;" class="button button-primary couponxl_pay_all_sellers" style="margin-left: 10px;"><?php _e( "Pay All Sellers", "couponxl" ) ?></a>' );;
							<?php } ?>
						});
					</script>
				<?php
		...
		function custom_bulk_action() {
			...
			if($post_type == $this->bulk_action_post_type) {
				...
				$sendback = esc_url( remove_query_arg( array('exported', 'untrashed', 'deleted', 'ids'), wp_get_referer() ) );
				if ( ! $sendback )
				...
				$sendback = esc_url( add_query_arg( array('success_action' => $action, 'ids' => join(',', $post_ids)), $sendback ) );
				
				$sendback = esc_url( remove_query_arg( array('action', 'paged', 'mode', 'action2', 'tags_input', 'post_author', 'comment_status', 'ping_status', '_status',  'post', 'bulk_edit', 'post_view'), $sendback ) );
