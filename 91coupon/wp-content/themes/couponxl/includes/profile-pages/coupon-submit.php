...
<div class="input-group">
    ...
    <select name="coupon_type" id="coupon_type" class="form-control" data-validation="required" data-error="<?php esc_attr_e( 'Select coupon type', 'couponxl' ); ?>">
    	...
    	<option value="code" <?php echo 'code' == $coupon_type ? 'selected="selected"' : ''; ?>><?php _e( 'Code', 'couponxl' ) ?></option>
    	<option value="sale" <?php echo 'sale' == $coupon_type ? 'selected="selected"' : ''; ?>><?php _e( 'Sale', 'couponxl' ) ?></option>
    	<option value="printable" <?php echo 'printable' == $coupon_type ? 'selected="selected"' : ''; ?>><?php _e( 'Printable', 'couponxl' ) ?></option>
    </select>
    <p class="description"><?php _e( 'Choose type of the coupon.', 'couponxl' ); ?></p>
</div>

<div class="input-group group_code <?php echo 'code' == $coupon_type ? 'visible' : ''; ?>">
...
<div class="input-group group_sale <?php echo 'sale' == $coupon_type ? 'visible' : ''; ?>">
...
<div class="input-group group_printable <?php echo 'printable' == $coupon_type ? 'visible' : ''; ?>">
    <label for="coupon_image"><?php _e( 'Printable Coupon Image', 'couponxl' ); ?> <span class="required">*</span></label>
	<?php if( pbs_is_demo() ) :?>
        <input type="hidden" name="coupon_image" id="coupon_image" value="<?php esc_attr( $coupon_image ) ?>" data-error="<?php esc_attr_e( 'Please select image for the printable coupon', 'couponxl' ); ?>">
	<?php else: ?>
    ...
    <a href="javascript:;" class="image-upload <?php echo pbs_is_demo() ? '' : esc_attr( 'coupon-image' ); ?>"><?php _e( 'Select coupon image', 'couponxl' ) ?></a>
