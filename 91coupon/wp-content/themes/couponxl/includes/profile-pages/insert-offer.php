...
if( pbs_is_demo() ) {
	$message = '<div class="alert alert-danger">' . _e( 'This is demo and offer submitting is not possible', 'couponxl' ) . '</a></div>';
	exit;
} else {
...
if( !empty( $offer_submit_price ) ){
	...
	if( !isset( $response['error'] ) ){
		$message = '<div class="alert alert-info">'.__( 'Your offer is successfully submitted. In  order to be reviewed and approved you need to pay for its submission. You can do that by clicking on ', 'couponxl' ).''.$response.'</div>';
		if( function_exists( 'couponxl_new_offer' ) ) {
	...
	$message = '<div class="alert alert-success">'.__( 'Your offer is submitted and it will be reviewed as soon as possible', 'couponxl' ).'</div>';
	if( function_exists( 'couponxl_new_offer' ) ) {
		couponxl_new_offer( $offer_id );
	}
}
