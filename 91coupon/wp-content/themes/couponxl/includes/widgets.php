		...
		echo wp_kses_post( $before_widget.$before_title.$title.$after_title );
			...
			<?php if( $theme_usage == 'all' ): ?>
				<ul class="list-unstyled list-inline offer-type-filter">
					...
					<li>
						<a href="<?php echo esc_url( couponxl_append_query_string( $permalink, array( 'offer_type' => 'deal' )  ) ) ?>" class="<?php echo 'deal' == $offer_type ? 'active' : '' ?>"><?php _e( 'Deals', 'couponxl' ) ?></a>
					</li>
					<li>
						<a href="<?php echo esc_url( couponxl_append_query_string( $permalink, array( 'offer_type' => 'coupon' ) ) ) ?>" class="<?php echo 'coupon' == $offer_type ? 'active' : '' ?>"><?php _e( 'Coupons', 'couponxl' ) ?></a>
					</li>
				</ul>
			<?php endif; ?>
							...
							<li>
								<a href="<?php echo esc_url( couponxl_append_query_string( $permalink, array( 'offer_cat' => $offer_cat_item->slug ) ) ) ?>">
									<?php if( !empty( $icons ) && $icons == 'yes' ): ?>
										...
										<?php endif; ?>
									<?php endif; ?>
									<?php echo wp_kses_post( $offer_cat_item->name ); ?>
								</a>
							</li>
							<?php
						endif;
...
class Couponxl_Locations extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		...
		echo wp_kses_post( $before_widget.$before_title.$title.$after_title ) . '<div class="white-block-content">';
...
class CouponXL_Mega_Menu_Images extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		...
		echo wp_kses_post( $before_widget.$before_title.$title.$after_title );
			echo '<a href="'.esc_attr( $link ).'"><img src="'.esc_url( $image ).'" class="img-responsive" /></a>';
		echo wp_kses_post( $after_widget );
	}
 	public function form( $instance ) {
